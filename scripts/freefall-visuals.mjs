...
function flight(kind,p){
 ...
 const height=19.6*t-4.9*t*t,Y=420-height/19.6*300,v=19.6-9.8*t,X=430;
 let out=line(120,435,1100,435)+text('投げた位置',140,480,27)+line(X,420,X,120,C.dim,2)+circle(X,Y,19,C.gold)+arrow(120,350,120,120,C.dim)+text('上向きを正',45,85,28);
 if(kind==='fall-energy'){
  const f=height/19.6;
 ...
 if(kind==='fall-height')out+=arrow(300,420,300,120,C.purple)+text('高さの変化 Δy',650,330,30,C.purple);
...
export function freefallDiagram(kind,p){
 ...
 if(kind==='apex-continuous'||kind==='apex-crossing'){
  const u=ease(p),t=kind==='apex-crossing'?1.5+.5*u:4*u,x=190+190*t,y=110+85*t;
  return line(140,280,1080,280)+line(190,460,190,55)+text('速度',105,50,28)+text('時間',1010,325,28)+text('0',150,289,27)+text('正',145,120,27,C.cyan)+text('負',145,445,27,C.purple)+path([[190,110],[950,450]],C.cyan,4)+circle(x,y,13,C.gold)+text('正から負へ変わる途中で、ゼロを通る',230,505,29);
 }
