...
export function shmFoundationDiagram(kind,p){
 if(!shmFoundationKinds.includes(kind))throw Error('Unknown '+kind);const u=ease(p),time=clamp(p/.8);
 if(kind.startsWith('shm-')){
  ...
  const [a,b,title]=settings[kind];let out=text(title,175,35,31)+oscillator(a+(b-a)*time);
 ...
 if(kind==='wave-circle-height'||kind==='wave-signed-height'){
  const a=kind==='wave-signed-height'?Math.PI+Math.PI/2*time:2*Math.PI*time,x=300,y=270,r=155,X=x+r*Math.cos(a),Y=y-r*Math.sin(a);
