...
const expected=read('docs/video-revision-20260924/pilot-plan.json');
for(const clip of [...main,...inserts]){
 ...
 for(let i=0;i<clip.scenes.length;i++){
  ...
  for(const p of s.utterances??[]){assert.equal(p.subtitle.split('。').filter(Boolean).length,1);assert.equal(p.reading.split('。').filter(Boolean).length,1);assert.ok(!/電場|磁場|電気束|磁束|上向き|下向き|法線|一様|時定数|数密度/.test(p.reading),p.reading);}
  for(const eq of s.equations){assert.ok(!/\\(?:rm|mathrm)\s*\{?[^}]*[一-龠ぁ-んァ-ヶ]/.test(eq),eq);}
 }
}
