...
import json, subprocess, re, concurrent.futures, sys
from pathlib import Path
import imageio_ffmpeg
root=Path(__file__).resolve().parents[1]
catalog_path=sys.argv[1] if len(sys.argv)>1 else 'src/content/em-video-catalog.generated.json'
catalog=json.loads((root/catalog_path).read_text())
ffmpeg=imageio_ffmpeg.get_ffmpeg_exe()
def check(clip):
    file=root/'public/media'/clip.get('mediaDirectory','em')/f"{clip['id']}.mp4"
