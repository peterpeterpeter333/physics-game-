...
export const stages=chapters.filter(c=>['uem','ui-em','um-em'].includes(c.id)).flatMap(c=>c.stages.map(stage=>{
 ...
 return {id:stage.id,title:stage.title,level:c.level,steps:steps.map((s,i)=>({...s,narration:narration[i],movieStageId:stage.id,movieSceneIndex:i})),units:learningPaths[stage.id]};
}));

export function diagram(step:any,t:number){
 const sphereArea=step.movieStageId==='um-gauss-sphere-preview'&&step.movieSceneIndex===5;
 const scene=sphereArea?'sphere-area':step.cycle?scene3DFor(step.cycle,step.phase):undefined;
 const Figure=REGISTRY[step.figure];
 const shot=reviewShots[step.figure];
 const node=scene?<EMScene3D scene={scene} phase={sphereArea?3:step.phase} videoTime={t}/>:step.pathPart!==undefined?<PathMeaning initialPart={step.pathPart} videoTime={t}/>:shot?<svg viewBox="0 0 320 220">{shot.draw(Math.min(1,t/12),.55)}</svg>:Figure?<Figure/>:step.scene?<GuidedScene scene={step.scene} beat={step.beat??Math.min(step.phase??0,2)}/>:null;
