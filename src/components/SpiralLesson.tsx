...
function WorkPlot({kind,phase}:{kind:'constant'|'linear';phase:number}){
 const [n,setN]=useState(4),[L,setL]=useState(3);
 ...
 return <div className="work-plot"><svg viewBox="0 0 390 290" role="img" aria-label={kind==='constant'?'一定の力の長方形を分割する図':'直線的に増える力の仕事を近似する図'}>
  ...
  {Array.from({length:n},(_,i)=>{const a=i*L/n,F=kind==='constant'?2:2*a;return <rect key={i} x={x(a)} y={y(F)} width={95*L/n} height={225-y(F)} fill="#57dff835" stroke="#57dff8"/>;})}
 ...
 </svg><label>分割数 N = {n}<input aria-label="仕事の分割数" type="range" min="1" max="40" step="1" value={n} onChange={e=>setN(Number(e.target.value))}/></label><label>移動距離 L = {L.toFixed(1)} m<input aria-label="仕事の移動距離" type="range" min=".5" max="3" step=".5" value={L} onChange={e=>setL(Number(e.target.value))}/></label></div>;
}
...
export function SpiralLesson({stage,alreadyFinished,onComplete,onExit}:{stage:Stage;alreadyFinished:boolean;onComplete:(firstTime:boolean)=>void;onExit:()=>void}){
 ...
 return <div className="screen lesson guided-lesson question-lesson spiral-lesson" ref={anchor} data-spiral-lesson data-stage-id={stage.id}>
  ...
  <section className="question-picture" aria-label="図で確かめる" key={`${cycle.id}/${phase}`}>
   {card.lab?<WorkPlot kind={card.lab} phase={phase}/>:card.figure?<Figure id={card.figure}/>:card.scene?<><GuidedScene scene={card.scene} beat={Math.min(diagram,(source?.beats.length??3)-1)}/>{!['r-path','r-calculate','r-compare'].includes(card.scene)&&<label className="guided-camera">図を比較<input aria-label="図の段階を比較" type="range" min="0" max={(source?.beats.length??3)-1} step="1" value={diagram} onChange={e=>setDiagram(Number(e.target.value))}/></label>}</>:null}
  </section>
  {card.tex&&<div className="guided-equation"><EquationImage tex={card.tex}/></div>}
