...
const phases=['基本事項','ステップ1','ステップ2','つながったこと'];
export function SpiralLesson({stage,alreadyFinished,onComplete,onExit}:{stage:Stage;alreadyFinished:boolean;onComplete:(firstTime:boolean)=>void;onExit:()=>void}){
 ...
 return <div className="screen lesson guided-lesson question-lesson spiral-lesson" ref={anchor} data-spiral-lesson data-stage-id={stage.id}>
  ...
  <div className="spiral-context"><p className="spiral-goal">この段で求めること：{cycle.goal}</p><span>{cycle.uses}</span><nav aria-label="この段の学び方">{phases.map((p,i)=><button key={p} aria-current={phase===i?'step':undefined} onClick={()=>setPage(level*4+i)}>{p}</button>)}</nav></div>
  ...
  <nav className="question-navigation" aria-label="スライドを移動"><button className="btn btn-ghost" disabled={page===0} onClick={()=>setPage(p=>p-1)}>← 前へ</button><button className="btn btn-primary" onClick={()=>page===total-1?onComplete(!alreadyFinished):setPage(p=>p+1)}>{page===total-1?'バトルへ':phase===3?'次の基本事項へ →':phase===0?'ステップ1へ →':phase===1?'ステップ2へ →':'つながったことへ →'}</button></nav>
  <nav className="lesson-dots" aria-label="学びの段を選ぶ">{cycles.map((s,i)=><button className={`lesson-dot ${level===i?'active':''}`} key={s.id} aria-label={`${i+1}. ${s.title}`} aria-current={level===i?'step':undefined} onClick={()=>setPage(i*4)}><span/></button>)}</nav>
  ...
  <details className="question-detail"><summary>学習のつながり</summary>{cycles.map((s,i)=><button className="question-index" key={s.id} onClick={()=>setPage(i*4)}>{i+1}. {s.title} → {s.gain}</button>)}</details>
 </div>;
}
