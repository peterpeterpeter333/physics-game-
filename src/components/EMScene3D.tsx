...
export function EMScene3D({scene,phase}:{scene:Scene3D;phase:number}){
 ...
 if(scene==='plane'){
  ...
  sphere([0,0,0],1);const t=parameter,dt=.23,dp=.3;
  ...
  mark([0,0,1.22],'北極',C.ink);caption='金の輪は緯線。北極に近づくほど輪の半径R sinθと、小片の横幅が小さくなります。';
  readout=`θ=${(t*180/Math.PI).toFixed(0)}° ／ 緯線の半径 ${(Math.sin(t)).toFixed(2)} R`;
  legend='青：半径 R　金：緯線と小面積 dA';slider={label:'北極からの角度 θ [rad]',min:.12,max:2.75,step:.01};
 ...
 return <div className="em3d" data-scene-3d={scene}>
  ...
  {scene==='sphere-area'&&<label>小片の経度 φ<input aria-label="小片の経度" type="range" min="0" max="5.9" step=".05" value={phi} onChange={e=>{setPlaying(false);setPhi(Number(e.target.value));}}/></label>}
