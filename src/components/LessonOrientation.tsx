...
export function LessonOrientation({ stageId, heading, page, total }: {
  stageId: string; heading: string; page: number; total: number;
}) {
  ...
  return <section className="lesson-orientation" aria-label="この章と今の話">
    ...
    <div className="lesson-current-label">今の話 <span className="lesson-card-count" aria-live="polite">{page + 1} / {total}</span></div>
    <h2>{heading}</h2>
  </section>;
}
