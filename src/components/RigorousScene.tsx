...
export function FieldLaboratory({scene,beat}:{scene:string;beat:number}){
 ...
 {frame(scene,beat,<>
  ...
  {text(240,345,'ここまでに電気力がした仕事（累積）',C.ink,16)}
...
export function RigorousScene({scene,beat}:{scene:string;beat:number}){
 ...
 case 'r-voltage':art=<>{text(240,28,'外向きの電場へ電子を運ぶ',C.ink,17)}{many(5,i=><g>{circ(120,143,20+i*15,C.muted)}{text(328,70+i*34,`半径${i+1} → Vは低く`,i===b+1?C.p:C.muted,14)}</g>)}{arr(132,143,233,143,C.e)}{dot(146+b*22,143,'−',C.ink)}{arr(146+b*22,157,113+b*22,157,C.f)}{text(240,260,b===2?'電子：ΔVは負、ΔUは正、電気力の仕事は負':'等電位線：同じ高さ。電場は低い電位へ向く。',C.p,14)}</>;break;
