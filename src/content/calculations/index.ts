...
import { levelCalculations } from '../university-levels';
export const calculationRules = [...mechanicsCalculations, ...fieldCalculations, ...mathCalculations, ...universityCalculations, ...emCalculations, ...reviewCalculations, ...levelCalculations].map(rule=>({...rule,headings:rule.headings.filter(h=>!retiredEmHeadings.has(h))})).filter(rule=>rule.headings.length);
